...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int selectedTabText = 0xFF666666;
	...
	private List<ImageButton> button = new ArrayList<ImageButton>();
			...
			public void onClick(View v) {
				if (v instanceof ImageButton) {
					for (int i = 0; i < button.size(); i++) {
						button.get(i).setImageResource(
								((IconTabProvider) pager.getAdapter())
										.getPageIconResId(i));
					}
					((ImageButton) v).setImageResource(((IconTabProvider) pager
							.getAdapter()).getSelectedIconResId(position));
				} else if (v instanceof TextView) {
					for (int i = 0; i < texts.size(); i++) {
						texts.get(i).setTextColor(tabTextColor);
						
						
					}
					((TextView) v).setTextColor(selectedTabText);
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof ImageButton) {
				if (i == 0) {
					...
					button.get(0).setImageResource(
							((IconTabProvider) pager.getAdapter())
									.getSelectedIconResId(0));
			...
			if (v instanceof TextView) {
				...
				if (i == 0) {
					tab.setTextColor(selectedTabText);
	...
	/*	rectPaint.setAlpha(40);
        canvas.drawOval(new RectF(lineLeft+30, (height - 90)/2, lineRight-30, (height + 90)/2), rectPaint);*/
		// draw underline
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			if (pager.getAdapter() instanceof IconTabProvider) {
				...
				button.get(position).setImageResource(
						((IconTabProvider) pager.getAdapter())
								.getSelectedIconResId(position));
				...
				texts.get(position).setTextColor(selectedTabText);
